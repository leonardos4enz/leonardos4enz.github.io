$(document).ready(async () => {
    ...
    const initHabilidadesProgramacion = () => {
        const habilidades = [
    ...
    const initLineaAprendizaje = () => {
        ...
        const crearPaso = (fecha, descripcion, habilidades) => `
    ...
    const initHabilidadesBlandas = () => {
        ...
        const crearFila = (categoria, habilidades) => `
            <div class="col-6 my-4">
                <h4>${categoria}</h4>
                <div class="d-flex flex-wrap gap-2 justify-content-center">
                    ${habilidades.map(habilidad => `<span class="badge rounded-pill bg-light border border-secondary px-3 py-2 text-dark">${habilidad}</span>`).join("")}
                </div>
            </div>
        `;
